/**
 * Created by tecWang on 2017/1/10.
 */
//好友列表数据
...
$(function () {
    ...
    $.each(userFaces, function (key,value) {
            var $face = $("<img src='images/faces/"+key+"' alt='"+value+"'>");
            $face.appendTo($faceDiv);
    });
    $(".faceBtn").append($faceDiv)
        .hover(function () {
            $($faceDiv).show()
                .find("img").click(function () {
                    var str = "[" + $(this).attr("alt") + "]";
                    $(".msgText").text($(".msgText").text() + str);
                });
        },function (){
            $($faceDiv).hide();
    });
    //添加朋友功能
    var $friendDiv = $("<div class='friendDiv'></div>");
    $.each(friendsList, function (key, value) {
        var $fri = $("<span>"+'@'+value+"</span>");
        $fri.appendTo($friendDiv);
    });
    $(".friendBtn").append($friendDiv)
        .hover(function () {
            $($friendDiv).show()

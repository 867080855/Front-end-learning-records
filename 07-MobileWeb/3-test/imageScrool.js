...
// this.width = this.config.width||this.box.children().eq(0).width();//一次滚动的宽度
        ...
        this.changeSize = this.changeSize || true; // 默认自动适应屏幕宽度
    ...
    $.extend(mggScrollImg.prototype,{
        ...
        change:function (e) {
            if(this.changeSize){
                ...
                var imgs = document.querySelectorAll('.comshowimg .imgbox');
                for(var x = 0; x < imgs.length; x++){
                    imgs[x].style.width = width + 'px';
                    imgs[x].children[0].style.width = width + 'px';
                }
            }
        },
        getWidth:function () {
